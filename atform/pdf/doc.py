...
def build_init_data():
    """Constructs the arguments for init().

    This must be called by the parent process, i.e., not the workers,
    when building via multiprocessing to ensure the captured data sets are
